...
Temp = 1000;%Temperature in Kelvins
...
%dt = 0.000005; %timestep taken each iteration , dt = 0.00005;
dt = 0.00003; %timestep taken each iteration , dt = 0.00005;
...
for t = 0:dt:max(beamPath)
    ...
    for i = 1:grainCount-1
        ...
        if growthRateLocal>scanSpeed*  0.5
            growthRateLocal = scanSpeed* 0.5;
        elseif growthRateLocal<scanSpeed* 0.1
            growthRateLocal = scanSpeed* 0.1;
        end

        %If the nuclei doesn't have any points on the grid, stop it from
        %growing
