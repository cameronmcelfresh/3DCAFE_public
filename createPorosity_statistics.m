function [gridPorous,PSD] = createPorosity_statistics(grid,xgrid,ygrid,zgrid,Tm,maxTemps,powderSize,bins,numTrials)
...
PSD = zeros([length(bins)-1,numTrials]);
for j = 1: numTrials
    [C,R,~,~] = spherePacking(300,R_mean,R_STD,maxIters); %create packed sphereical volume
    PSD_sample = histcounts(R,bins,'Normalization','probability');
    PSD(:,j)=PSD_sample';
end
