function [growthSlice,growthIndex] = findCAGrowthSlice_timed(xIndex,yIndex,grid,gridSize,posInd,gridSolidifyTime, growthRate,realGridSize,t)
%FINDCAGROWTHSLICE Function to find the growth slice using CA and 
%"growthRate" here is given as cell distance / s
growthSlice = [];
growthIndex = [];

%Distance grown
distGrown=growthRate*(t-gridSolidifyTime(posInd)); %distance grown in the dt time
...
for step=1:length(possibleSteps)
    ...
    if (yIndex+possibleSteps(step,2)>0 && yIndex+possibleSteps(step,2)<gridSize+1 && ...
        ...
        if grid(yIndex+possibleSteps(step,2),xIndex+possibleSteps(step,1))==0
            ...
            %Check if the distance grown is more than the pixel-pixel
            %distance
            if distGrown>norm(possibleSteps(step,:))
